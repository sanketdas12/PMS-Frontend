...
@Component({
  selector: 'app-assign-pay',
  standalone: true,
  imports: [CommonModule, FormsModule, SkeletonComponent],
  templateUrl: './assign-pay.component.html',
  styleUrl: './assign-pay.component.css'
})
export class AssignPayComponent implements OnInit {
  ...
  assign() {
    ...
    this.assignService.assign({ empId: this.selectedEmpId, payStructureId: this.selectedStructId }).subscribe({
      ...
      error: err => {
        this.msg = err?.error?.message ?? 'Failed to assign.';
        this.msgType = 'error';
        this.loading = false;
      }
    });
  ...
  deleteAssignment(id: string) {
    this.assignService.delete(id).subscribe({ next: () => this.loadAssignments(), error: () => {} });
  }
